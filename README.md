...
- 如果原文含 `附注`、`Note`、`设计意图`、`个人意图` 等内容，普通强度评分仍按公开生态标准；另加 `设计相关评分 - 个人期望`，判断它是否达成作者自己的私桌目标。
...
- 对这种官方 RAW 模糊相关的强度 / 功能 / 综合评分，默认采用保守 RAI：承认清晰意图下的功能价值，但不把最宽松 RAW 套利作为默认强度基础，除非 homebrew 自己明确创造或要求该套利。
- 如果评审需要给出“官方 RAW 应如何修”的方案，放入独立 `## 官方RAW修复建议` section，并在目录中列出。该 section 只讨论官方规则修复口径及其桌面影响，不写成 homebrew 必须修改项。
