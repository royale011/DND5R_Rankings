...
- Open-ended boundary audit: unbounded-looking target counts, action-economy compression, spell copying, stored spells, broad ally benefits, or spell-slot-per-turn exceptions are investigation triggers rather than automatic defects. Verify current executable routes, costs, timing, counterplay, official ecosystem baselines, and whether the danger depends on extreme NPC/summon density, permissive DM setup, or future unreleased resources before changing rank or demanding a fix: official `Build the actual rules context before scoring`.
