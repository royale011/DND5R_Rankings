...
- Open-ended boundary audit and encounter/story counter calibration: unbounded-looking target counts, action-economy compression, spell copying, stored spells, broad ally benefits, spell-slot-per-turn exceptions, narrow hard counters, damage/condition immunities, always-on detection, high passive skill floors, divination, and other story-axis breakers are investigation triggers rather than automatic defects. Verify current executable routes, costs, timing, counterplay, official ecosystem baselines, campaign-frequency, DM encounter/story design burden, table-operation burden from repeated mass saves / tracking, raw frequency versus reliance, self-only versus party-wide scope, and whether the danger depends on extreme NPC/summon density, permissive DM setup, future unreleased resources, a narrow encounter category, or a specific campaign premise before changing rank or demanding a fix. Use the official skill's principle-level baselines and 5etools JSON lookup workflow instead of copying old single-example conclusions: official `Build the actual rules context before scoring`.
- Resource counting, action economy, multi-step setup reliability, maintained-state interruption risk, concentration, stored-spell timing and caster identity, same-subclass feature stacking, base-class chassis stacking, exact attack-wording distinctions, post-roll / post-failure correction math and official same-pattern timing inheritance, melee / short-range / multi-hit damage-increment resources, spell bounce / chain mechanics, Paladin smite / rider concentration checks, unwanted interruption, severe costs, and feature-overlap analysis: official `Build the actual rules context before scoring`.
- Legacy damage-aspect scoring, including `近战伤害者`, `远程伤害者`, `法术伤害者`, concrete calculation when damage materially affects rank, conditional extra attack opportunities as a correction layer inside existing damage / `团队增益` aspects, and the rule that six-category DPR calculation-table requirements are currently archived in `SKILL.dpr-next.md`: official `Use the same scoring system` and `Write feature-level reviews`.
...
Do not automatically transfer `祈愿术`'s historical-design tolerance to a separate homebrew feature that grants arbitrary spellcasting, such as "cast any spell of 5th level or lower." A non-Wish arbitrary spell pool is its own rules object. Audit its spell-list boundary or lack of one, maximum spell level, action timing, no-slot wording, material/component waiver, concentration, target/range/duration inheritance, excluded triggers such as smite-like follow-up spells, public lookup burden, and whether a fixed spell list or narrower class-list restriction would improve public readability. This audit is not an automatic design penalty; a feature can remain `强但可接受` when its level, cooldown, resource gate, state-ending cost, and inherited spell limits are real.
...
For homebrew design scoring, use the same concise-text principle as the official skill: inherited official rules should remain concise, current official same-pattern wording is strong evidence that a phrase is clear enough, and self-made exceptions must define the new boundary they create. Do not require a homebrew to replace an official-pattern phrase with a more explicit alternative unless the homebrew changes the official pattern's behavior or creates a real execution ambiguity. A formal sidebar, published rule note, or clearly included rules appendix can protect `能力设计质量` when it explains a complex exception without bloating the main feature. Ordinary `附注`, task notes, prompts, blind-review responses, private discussion, or author meta-commentary do not repair public-facing rules text unless the source or user explicitly says that material is meant to be published with the subclass.

Verified official rules terms are normal vocabulary in homebrew as well. If a term is confirmed in the current rules glossary, condition rules, action rules, monster/stat-block terminology, spell rules, item rules, or another official rules location, do not require a homebrew-specific definition merely because the term is compact. Explain it briefly in the review only when useful for readers. Require extra wording only if the homebrew changes that official meaning, creates a new exception around it, or uses the term in a way the official definition cannot execute.

Also use the official skill's RAI / table-adjudication-room distinction. A visible, narrow, costly, late-tier, easy-to-adjudicate edge can be healthy concise design. Do not lower a homebrew's design score merely because a DM must decide a rare boundary case. Lower the score when the ambiguity hides a major power mode, changes resource ownership or action economy, creates a loop, determines target legality, or requires repeated negotiation during normal use.
...
When comparing a homebrew feature to a verified official / UA / partner / third-party reference, state both the homebrew's major gains and its real costs, limits, regulators, timing restrictions, resource pressure, or counterplay before calling it too strong, too weak, acceptable, or unclear. This does not require a formal comparison table; include it naturally in `详细评价`, `横向对照与不确定点`, or `修改建议`.

When auditing broad target ranges, repeatable triggers, missing hard caps, or open resource loops, use neutral boundary accounting. Do not phrase a review as if a feature "should have" a long-rest cap, PC-only target, smaller ally count, narrower range, concentration, or other limiter merely because those limiters are common safety tools or appeared in an older draft / user discussion / blind review. First evaluate the current text's actual executable frequency, costs, action economy, resource gates, target availability, encounter frequency, counterplay, and table workload. Distinguish expanding a disclosed scope from creating an uncontrolled boundary: changing a target from `player character ally` to `ally`, or removing an extra long-rest limit from a feature already gated by spell slots, class resources, reaction, active state, positioning, and trigger timing, should be scored by the concrete added cases rather than treated as inherently open-ended. For NPCs, mounts, summons, and story allies, ask whether they realistically add spotlight-stealing actions, large extra DPR/control, or bookkeeping in a normal table; do not assume either zero value or adversarial NPC/summon swarms. If the current feature is still healthy, describe the broad boundary as acceptable or as a monitoring point. If it is too strong or too hard to run, explain the concrete route that makes it so. Design-score penalties must come from the current feature's real public execution burden, abuse path, or niche compression, not from the absence of a hypothetical nerf.

When a feature grants, spends, transfers, or improves access to an official resource, condition, rule object, or named game term, verify that object's own eligibility limits before judging the homebrew boundary. If the underlying object is limited to PCs, creatures, allies, spellcasters, weapon users, a specific action, a specific rest cycle, or another official scope, the homebrew normally inherits that scope unless it clearly overrides it. Expanding a feature's target words beyond the underlying object's eligible scope can be correct when the extra targets still cannot receive or use the resource, but it needs auditing when the wording actually bypasses the object's verified limit. Example principle: if a resource can only be acquired or held by a PC under the verified 2024 rule, a feature that lets an `ally` gain that resource still only works for allies who can legally gain it; if the feature explicitly makes non-PC allies able to gain or spend it, that is a real expansion and must be scored as such.

Do not let memory of an "expected" or "safer" version become an invisible baseline. This includes not only archived drafts, but also any remembered shape from prior conversations, blind-review experiments, previous review conclusions, author debates, or an internalized idea that the feature "normally would" have a specific limiter. When a review says a boundary is wide or a feature is costly to run, it must be because the current formal text creates a concrete, presently executable issue under the shared official / UA / partner / third-party calibration pool. If the same sentence would read as "this is worse because it is not the older / safer / expected version," rewrite it. Current text plus verified ecosystem baselines are the only public-score comparison points.
...
3. Read the source homebrew file.
   ...
   - Exception for explicitly non-public material: if the source note, user task, or prompt clearly says the material is not public-facing / not player-available / only a private NPC, boss, setting, archive, or experiment, then the note defines the review mode instead of being ignored. In that case, do not force a public-player ecosystem review, do not generate public strength ranks, `分项评分`, `综合评分`, or public-release modification demands unless the user explicitly asks for them. Write the review as a private/design/completeness review, following the stated task boundary and using normal rules analysis only where it helps explain coherence, executability, or table handling. This exception is narrow: ordinary design notes, author preferences, blind-review tasks, or desired outcomes still do not control public rankings for material that is otherwise being reviewed as a public homebrew.
...
6. Write the review.
   ...
   - Include `原文`.
   - Include team/rules position, feature-level `详细评价`, per-tier `分项评分`, vertical `综合评分`, `设计相关评分`, official baseline comparison / uncertainty notes, damage calculation discussion when the homebrew materially affects damage ranking, and `修改建议`.
