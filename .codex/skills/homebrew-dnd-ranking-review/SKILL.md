...
- Legacy damage-aspect scoring, including `近战伤害者`, `远程伤害者`, `法术伤害者`, concrete calculation when damage materially affects rank, and the rule that six-category DPR calculation-table requirements are currently archived in `SKILL.dpr-next.md`: official `Use the same scoring system` and `Write feature-level reviews`.
